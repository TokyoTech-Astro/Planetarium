...
export default function MotorManage() {
  const onSubmit = async (formData: FormData) => {
    ...
    if(deg >= 0) await axios.post(`http://pi-starsphere.local:8000/motor?dir=forward&deg=${deg}&speed=${speed}`)
    else if(deg < 0) await axios.post(`http://pi-starsphere.local:8000/motor?dir=back&deg=${-deg}&speed=${speed}`)
  }
